...
An earlier section closed by saying the mechanics were verified while the evolutionary result remained open. That gap has since been closed, and the way it closed is the rest of this essay's argument.

## Registered questions, registered answers

With the ecology fixed and audited, the project moved to its first actual evolution questions, run under a discipline borrowed from clinical trials: every question is written down, with its decision rule and its failure interpretation, before any data-producing execution is allowed. The rule text, the seed tables, the window length, and the code itself are hash-frozen into a manifest before the experiment starts; the analysis program refuses to run twice; and nothing may be changed after results arrive except by writing a new, superseding registration that says exactly what changed and why.

Stage 7 asked whether allocation strategy differences travel through the population's causal channel at all — whether surplus conversion traits change lifecycle outcomes under exogenous hazard. A long chain of preregistered assays established the positive half: the channel exists. Three separate repair designs proposed along the way were caught by executable gates and archived, unpublished but preserved, in the repository's failed-designs record. Then came the question one level up: does evolution use the channel? Two preregistered experiments asked it, at two endpoint families, and both returned the same disciplined answer: not established.

The first compared growth-rate roots between hazard exposures and found no established direction, with a recorded cross-sectional slope bound near 8×10⁻⁵ allocation units per unit of exposure contrast.

The second opened a mutation kernel on the allocation locus itself — the project's first genuine evolution run. Each birth flips allocation by one to four lattice steps with probability one half, clamped to the legal range; twenty-four replicate pairs ran 2,400 ticks each, the mutated arm against a byte-frozen mutation-off reference at identical seeds. The feasibility gate caught a real defect before any retained data existed: the gate tooling's bookkeeping expectation disagreed with the frozen stack's closure semantics by one entry, and the mismatch was diagnosed, archived, and repaired by a superseding registration that changed only that operationalization. The corrected gate passed; thirty files were hash-pinned; one authorised execution ran to completion — 48 populations, zero extinctions, 23,933 kernel events on the mutated arm, and a mutation-off reference whose zero-draw state is asserted on every run.

The decision rule then did the thing it was built to do. Pairwise terminal-allocation differences had exactly the magnitude a real effect would need — the median absolute difference sat right at the pre-registered significance floor — but their signs split nearly evenly, thirteen against eleven, where the rule demanded eighteen pairs agreeing in one direction. The registered outcome was *no established direction*. Not "we found nothing because we looked weakly": dispersion was measured, the test's false-positive rate is fixed by arithmetic regardless of dispersion, and the failure was concordance, not magnitude. The licensed conclusion is a bound: at this ecology, this kernel, and this window, evolution through the channel does not redistribute terminal allocation beyond about ±1.6% of the allocation scale per replicate pair.

This is the completing move for the essay's thesis, not a contradiction of it. Conservation is not an ecology; verified channels are not selection. You can conserve every resource unit, pass every independent audit, catch your own gate's bug before it matters, execute exactly once exactly as registered — and the honest answer can still be a bounded negative. What the discipline buys is not a positive result. It buys knowing precisely which negative result you have: which claims are excluded, at what resolution, with what measured background dispersion (here, a paired standard deviation near five and a half lattice units) left behind as a quantified starting point for whoever asks the next question with different tools — trajectory instruments, other ecologies, open populations — none of which this architecture's statement space was ever allowed to license.
