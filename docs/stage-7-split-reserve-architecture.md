...
**Status:** isolated Slice 1 and mechanics-only population Slice 2A are implemented under explicit provisional defaults; mutation, scientific preregistration, and every Stage 7 assay remain **NO-GO** pending §13.
...
Before any assay, separately resolve and preregister: (a) the legal `A/T` strategy set, starting distribution, mutation kernel, boundaries, and resolution sensitivity; (b) whether `R` storage has upkeep or decay on longer timescales; (c) whether vacancy capture is part of the estimand or admission is independently standardised/nonbinding; (d) the primary invasion-growth/reproductive-value endpoint and cohort mediators; and (e) the treatment contrast, falsification gates, and source-frozen analysis plan. Slice 1 and Slice 2A mechanism code and traces are authorized; evolutionary simulation, confirmatory preregistration, and every scientific assay remain unauthorized.
