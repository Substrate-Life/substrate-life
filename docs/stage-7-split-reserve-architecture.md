...
`stage-7b-fixed-allocation-channel-preregistration.md` now freezes a narrower scripted mechanism verification at `A∈{102,204}`, `T=128`, and `D=255`. It explicitly discloses prior deterministic calibration, contains no fitness endpoint, and authorises no execution by itself. Before its first registered execution, its implementation, runner, tests, schema, analysis, and hashes must be frozen together. Before any later population-fitness assay, separately resolve and preregister: (a) the legal `A/T` strategy set, starting distribution, mutation kernel, boundaries, and resolution sensitivity; (b) whether `R` storage has upkeep or decay on longer timescales; (c) whether vacancy capture is part of the estimand or admission is independently standardised/nonbinding; (d) the primary invasion-growth/reproductive-value endpoint and cohort mediators; and (e) the treatment contrast, falsification gates, and source-frozen analysis plan. Slice 1 and Slice 2A mechanism code and traces are authorized; Stage 7B0 implementation/execution, evolutionary simulation, confirmatory population inference, and every scientific assay remain unauthorized.
