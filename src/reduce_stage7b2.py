"""Stage 7B2 reducer: independent recomputation and the single application
of the registered Section 5 decision rule of
``docs/stage-7b2-preregistration.md``.

Reads the retained raw artifact produced by ``run_stage7b2.py``, rebuilds the
exact cohort schedules from the retained vital records, re-runs the
...
def _source_hashes() -> dict[str, str]:
    ...
    for filename in REDUCER_SOURCES:
        if filename == "reduce_stage7b2.py":
            path = __file__
        else:
...
def reduce_artifact(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    reduced: dict[str, Any] = {
        "protocol": "stage-7b2-preregistration",
        "evidence_class": "single application of the registered section 5 "
        ...
        "interpretation_limits": (
            ...
            "registered floor under the registered ecology; it does not "
            "establish an optimum, an ESS, a background-invariant causal "
            "effect, or the architecture section 9.5 external-validation "
            "mechanism."),
    }
    return reduced
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("raw", type=str,
                        help="retained raw artifact from run_stage7b2.py")
