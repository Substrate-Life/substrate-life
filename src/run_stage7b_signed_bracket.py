"""Stage 7B signed-bracket confirmatory runner: executes the registered
k = 32 replicate suite at the carried section 3 ecology under the
two-factor endpoint, certifying FULL-LINE Euler-Lotka brackets
(``docs/stage-7b-signed-bracket-preregistration.md``), and exports raw
measurements under ``results/stage7b-signed-bracket/``.

Authorisation boundary (signed-bracket prereg sections 5, 6 and 8): this
runner may only be executed for retention AFTER (a) the section 5
feasibility gate has passed on its reused fixed shakedown table and (b)
the single-commit freeze whose manifest is
``results/stage7b-signed-bracket/pre-execution-manifest.json`` is
committed. During the implementation window only the section 5
exploratory, NON-RETAINED shakedowns (``stage7b_signed_bracket_gate.py``)
are sanctioned; any other execution at the registered ecology is
unauthorised by section 8.

The runner applies no decision rule. It measures with the unchanged
two-factor endpoint (``stage7b_exposure_measure.exposure_schedule``;
``m^E_x`` / ``l^A_x``; the establishment/first-reproduction quantity stays
a reported mediator), certifies FULL-LINE solver brackets with the new
additive solver (``stage7b_signed_bracket_solver.full_line_certified_bracket``),
records classifications and identities, and writes one JSON artifact
whose schema is fixed in ``docs/stage7b-signed-bracket-output-schema.md``.
The completed section-3-table decision rule is applied exactly once by
``reduce_stage7b_signed_bracket.py``, the source-frozen reducer. Every
prior frozen module is reused unmodified.
"""
...
#: (signed-bracket prereg Authorisation section); only the configuration,
#: full-line solver, runner, reducer, gate, tests, and schema are new.
FROZEN_SOURCES = (
...
def _source_hashes() -> dict[str, str]:
    ...
    for filename in FROZEN_SOURCES:
        if filename == "run_stage7b_signed_bracket.py":
            path = __file__
        else:
...
def run_replicate(index: int) -> dict[str, Any]:
    ...
    for genotype_a in cohort_genotypes(vitals):
        ...
        schedules[genotype_a] = {
            ...
            "d_x": schedule["d_x"],
            "e_x": schedule["e_x"],
            "l_actuarial_x": [fmt_rat(v) for v in schedule["l_actuarial_x"]],
            "m_exposure_x": [fmt_rat(v) for v in schedule["m_exposure_x"]],
            ...
            "person_ticks_credited": schedule["person_ticks_credited"],
        }
...
def run_replicate_guarded(index: int) -> dict[str, Any]:
    ...
    architecture section 9 repair policy instead of losing the whole
    suite.  Used identically by the sequential and parallel execution
    paths.
    """
    try:
...
def _mediator_summary(population: Any, vitals: dict[str, Any]) -> dict[str, Any]:
    """Reported mediators via the frozen summary; never substituted for the
    endpoint (signed-bracket prereg carried section 3)."""
...
def _serialise_certificate(certificate: dict[str, Any]) -> dict[str, Any]:
    ...
    if "reason" in certificate:
        out["reason"] = certificate["reason"]
...
def main(argv: list[str] | None = None) -> int:
    ...
    raw: dict[str, Any] = {
        ...
        "evidence_class": "seeded, mutation-disabled confirmatory "
                          ...
                          "(N=48, E=900, W=1200); endpoint measured with the "
                          "unchanged two-factor coefficients "
                          "(l^A_x * m^E_x); solver certified on the FULL "
                          "real line; decision deferred to the reducer",
        ...
        "execution_class": "one seeded confirmatory suite (signed-bracket "
        ...
        "decision_scope": (
            "Raw measurement export only. The completed section-3-table "
            "decision rule (docs/stage-7b-signed-bracket-preregistration.md "
            "section 3, replacing only the joint-supercriticality pairing "
            "precondition of docs/stage-7b2-preregistration.md section 5 as "
            "carried) is applied exactly once by the source-frozen reducer "
            "reduce_stage7b_signed_bracket.py; nothing here establishes "
            "fitness, selection, an optimum, an ESS, or any "
