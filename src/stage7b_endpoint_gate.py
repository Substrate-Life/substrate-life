"""Stage 7B endpoint-repair feasibility gate (prereg section 5).

Tooling for the BINDING pre-freeze feasibility gate registered at
``docs/stage-7b-endpoint-repair-preregistration.md`` section 5: exploratory,
NON-RETAINED shakedowns at the exact carried section 3 configuration on the
**same fixed 24-seed table already used and archived by the failed gate**
(``20270000 + j``, ``j`` in ``0..23``; no new seed draw is needed or
permitted, section 5.2).  Produces no retained artifact: everything is
...
Gate conditions (all mandatory, unchanged from
``stage-7b2-repair-preregistration.md`` section 6, re-derived from the
carried section 5 rule's own precondition, endpoint-repair prereg
section 5.3):

- **G1** each genotype is supercritical (``L(0) > 1``, certified by the
  frozen solver contract under the corrected raw-fecundity ``m_x``)
  individually in at least two-thirds of shakedown replicates;
...
is a further superseding preregistration with a new diagnosis
(section 5.4).  This script never applies the carried section 5 decision
rule and makes no fitness, selection, or evolutionary claim; ``L(0)``
statuses are feasibility facts about the registered ecology, measured with
the exact frozen solver over the corrected endpoint.
"""
...
def run_shakedown(seed: int) -> dict[str, Any]:
    """One unretained shakedown replicate under the corrected endpoint."""
    ...
    try:
        ...
        establishments_credited: dict[str, int] = {}
        ...
        for genotype_a in cohort_genotypes(vitals):
            ...
            establishments_credited[str(genotype_a)] = \
                schedule["establishments_credited"]
        ...
        record.update({
            ...
            "establishments_credited": establishments_credited,
...
def evaluate_gate(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    summary: dict[str, Any] = {
        ...
        "failure_guidance": (
            ...
            "preregistration with a new diagnosis.  Shakedown executions "
            "produce no retained artifact (section 5.5)."),
