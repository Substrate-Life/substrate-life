...
def conditional_efficiency_assay_genome(
    ...
    result.extend([
        (SUB, 7, 4, 656),       # R7 = measured replenishment R4 - 656
...
class InstructionEngine:
    ...
    def _execute_instr(self, org: Organism, opcode: int, args: tuple) -> bool:
        ...
        if opcode == NOP:
            ...
            stillbirths_before = org.stillbirths
            offspring_id = self.substrate.reproduce(org)
            stillbirth = org.stillbirths > stillbirths_before
            self.substrate.divide_event_log.append({
...
class Simulation:
    ...
    def __init__(self, seed: int = 42, phase_mode: str = 'long',
        ...
        self.tick = 0
        self.metrics = {
            "births": 0, "deaths": 0, "stillbirths": 0,
    ...
    def step(self) -> int:
        ...
        still_warm = []
        for size, t in self.substrate.corpse_pool:
            if self.tick - t >= CORPSE_POOL_TTL:
                self.substrate.shared_memory_pool += size
            else:
                still_warm.append((size, t))
        self.substrate.corpse_pool = still_warm
    ...
    def run(self, max_ticks: int = 50000) -> dict:
        ...
        self.metrics["stillbirths"] = self.substrate.stillbirths
        return self.metrics
...
if __name__ == "__main__":
    ...
    print(f"Done: ticks={sim.tick}, max_pop={sim.metrics['max_population']}, "
          ...
          f"births={sim.substrate.births}, stillbirths={sim.substrate.stillbirths}")